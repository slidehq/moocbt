...
```bash
sudo modprobe moocbt
```

If you would like to have the module be loaded automatically during boot, consult the documentation for your distribution.

### Troubleshooting
On some systems, it may be necessary to let the system know of the location of the shared libraries. If you are having trouble getting `moo` to run, run these two commands:
```
echo /usr/local/{lib,lib64} | sed 's/ /\n/g' | sudo tee /etc/ld.so.conf.d/moocbt.conf
sudo ldconfig
```

### Usage
The kernel module is primarily controlled through `moo(8)`, which was installed previously. For usage instructions, see [moo.8.md](doc/moo.8.md).
