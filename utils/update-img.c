...
static int verify_files(FILE *cow, unsigned minor){
	...
	info = malloc(sizeof(struct moocbt_info));
	if(!info){
		ret = ENOMEM;
		errno = 0;
		fprintf(stderr, "error allocating mmeory for moocbt info\n");
		goto error;
	}

	//read info from the moocbt driver
	ret = moocbt_info(minor, info);
	if(ret){
		ret = errno;
		errno = 0;
		fprintf(stderr, "error reading moocbt info from driver\n");
		goto error;
	}
...
int main(int argc, char **argv){
	...
	ret = sscanf(snap_path, "/dev/moocbt%u", &minor);
	if(ret != 1){
		ret = errno;
		errno = 0;
		fprintf(stderr, "snapshot does not appear to be a moocbt snapshot device\n");
		goto error;
	}
