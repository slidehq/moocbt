...
class TestSetup(DeviceTestCase):
    ...
    def test_setup_invalid_minor(self):
        self.assertEqual(moocbt.setup(1000, self.device, self.cow_full_path), errno.EINVAL)
    ...
    def test_setup_volume_path_is_dir(self):
        self.assertEqual(moocbt.setup(self.minor, self.mount, self.cow_full_path), errno.ENOTBLK)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(moocbt.info(self.minor))

    def test_setup_cow_file_path_is_dir(self):
        self.assertEqual(moocbt.setup(self.minor, self.device, self.mount), errno.EISDIR)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(moocbt.info(self.minor))

    def test_setup_cow_file_on_wrong_device(self):
        self.assertEqual(moocbt.setup(self.minor, self.device, "/tmp/{}".format(self.cow_file)), errno.EINVAL)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(moocbt.info(self.minor))
    ...
    def test_setup_unmounted_volume(self):
        ...
        self.assertEqual(moocbt.setup(self.minor, self.device, self.cow_full_path), errno.EINVAL)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(moocbt.info(self.minor))
    ...
    def test_setup_already_tracked_volume(self):
        ...
        self.assertEqual(moocbt.setup(self.minor, self.device, self.cow_full_path), errno.EBUSY)
        self.assertTrue(os.path.exists(self.snap_device))
        self.assertIsNotNone(moocbt.info(self.minor))
