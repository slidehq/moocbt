...
class TestTransitionToIncremental(DeviceTestCase):
    ...
    def test_transition_nonexistent_snapshot(self):
        ...
        self.assertEqual(moocbt.transition_to_incremental(self.minor), errno.ENOENT)
    ...
    def test_transition_active_incremental(self):
        ...
        self.assertEqual(moocbt.transition_to_incremental(self.minor), 0)
        self.assertEqual(moocbt.transition_to_incremental(self.minor), errno.EINVAL)

    def test_transition_fs_sync_cow_full(self):
        scratch = "{}/scratch".format(self.mount)
        falloc = 50

        self.assertEqual(moocbt.setup(self.minor, self.device, self.cow_full_path, fallocated_space=falloc), 0)
        self.addCleanup(moocbt.destroy, self.minor)
    ...
    def test_transition_mod_sync_cow_full(self):
        ...
        err = moocbt.transition_to_incremental(self.minor)
        if (err != errno.EFBIG):
